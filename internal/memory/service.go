...
func (s *MemoryService) Store(ctx context.Context, m *Record) error {
	...
	logger.Debugf("[memory] storing memory for userID=%s type=%s", m.UserID, m.MemoryType)
	if err := s.qdrant.UpsertMemory(ctx, m); err != nil {
		return fmt.Errorf("store memory for userID=%s: %w", m.UserID, err)
	}
	logger.Debugf("[memory] successfully stored memoryID=%s for userID=%s", m.ID, m.UserID)
	return nil
}
...
func (s *MemoryService) Search(ctx context.Context, userID string, query string, opts *SearchOptions) ([]*Record, error) {
	...
	logger.Debugf("[memory] searching for userID=%s query=%.50s topK=%d", userID, query, opts.TopK)
...
func (s *MemoryService) GetProfile(ctx context.Context, userID string) (*Profile, error) {
	...
	logger.Debugf("[memory] retrieved profile for userID=%s messageCount=%d", userID, profile.MessageCount)
	return profile, nil
}

func (s *MemoryService) UpdateProfile(ctx context.Context, p *Profile) error {
	logger.Debugf("[memory] updating profile for userID=%s", p.UserID)
	if err := s.qdrant.UpsertProfile(ctx, p); err != nil {
		return fmt.Errorf("update profile for userID=%s: %w", p.UserID, err)
	}
	logger.Debugf("[memory] successfully updated profile for userID=%s", p.UserID)
	return nil
}
...
func (s *MemoryService) DeleteMemory(ctx context.Context, memoryID string) error {
	...
	logger.Infof("[memory] successfully deleted memoryID=%s", memoryID)
	return nil
}

func (s *MemoryService) DeleteUserData(ctx context.Context, userID string) error {
	logger.Warnf("[memory] deleting all data for userID=%s", userID)
...
func (s *MemoryService) ConsolidateWithMessages(ctx context.Context, userID string, messages []*DiscordMessage) error {
	logger.Infof("[memory] starting consolidation for userID=%s with %d messages", userID, len(messages))
	if err := s.consolidator.ProcessWithMessages(ctx, userID, messages); err != nil {
		return err
	}
	logger.Infof("[memory] completed consolidation for userID=%s", userID)
	return nil
}
...
func (s *MemoryService) IncrementMessageCount(ctx context.Context, userID string) (int, error) {
	...
	logger.Debugf("[memory] userID=%s count=%d/%d", userID, count, s.consolidationInterval)

	if count%s.consolidationInterval == 0 {
		profile, err := s.GetProfile(ctx, userID)
		if err != nil {
			logger.Warnf("[memory] failed to get profile for userID=%s: %v", userID, err)
			return count, nil
		}
		...
		if err := s.UpdateProfile(ctx, profile); err != nil {
			logger.Warnf("[memory] failed to update profile for userID=%s: %v", userID, err)
		}
	}
...
func (s *MemoryService) ConsolidateChannel(ctx context.Context, channelID string, messages []*DiscordMessage) error {
	logger.Infof("[memory] starting batch consolidation for channel=%s with %d messages", channelID, len(messages))
	...
	logger.Infof("[memory] completed batch consolidation for channel=%s", channelID)
	return nil
}
