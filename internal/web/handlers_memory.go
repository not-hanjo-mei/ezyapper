...
const (
	// defaultMemoriesLimit is the default page size for memory listing.
	defaultMemoriesLimit = 50
	// defaultMaxMessages is the default number of messages to fetch for consolidation.
...
func (s *Server) triggerConsolidation(c *gin.Context) {
	...
	go func() {
		...
		if channelID != "" && s.discordFetcher != nil {
			messages, err := s.discordFetcher.FetchUserMessages(ctx, channelID, userID, maxMessages)
			if err != nil {
