...
func TestLoadPluginsFromDirCommandRuntimeWithoutLocalExecutable(t *testing.T) {
	...
	output, err := pm.ExecuteTool(context.Background(), "command-plugin", "echo_value", map[string]any{"value": "hello"})
	if err != nil {
	...
	if _, err := pm.ExecuteTool(context.Background(), "command-plugin", "echo_value", map[string]any{}); err == nil {
		t.Fatal("expected ExecuteTool to fail when required argument is missing")
	}

	envOutput, err := pm.ExecuteTool(context.Background(), "command-plugin", "check_env", map[string]any{})
	if err != nil {
...
func TestExecuteToolTimeout_ToolSpecWins(t *testing.T) {
	...
	_, err := pm.ExecuteTool(context.Background(), "mock-plugin", "mock-tool", map[string]any{})
	if err == nil {
...
func TestExecuteToolTimeout_SuccessWithToolTimeout(t *testing.T) {
	...
	result, err := pm.ExecuteTool(context.Background(), "mock-plugin", "mock-tool", map[string]any{})
	if err != nil {
...
func TestExecuteCommandTool_PerToolTimeout(t *testing.T) {
	...
	start := time.Now()
	_, err = pm.ExecuteTool(context.Background(), "timeout-plugin", "slow_tool", map[string]any{})
...
func TestConcurrentExecuteToolAndDisablePluginNoPanic(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		_, err := pm.ExecuteTool(context.Background(), "test", "tool", map[string]any{})
		errCh <- err
	}()
